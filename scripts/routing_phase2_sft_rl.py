...
def train_sft_router(traces):
    ...
    for ep in range(200):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(router.state_dict(), PROJECT / "router_adapters" / "sft_oracle_router.pt")
    ...
    # Reload best
    router.load_state_dict(torch.load(PROJECT / "router_adapters" / "sft_oracle_router.pt"))
    router.eval()
    return router
...
def train_reinforce_router(traces):
    ...
    torch.save(policy.state_dict(), PROJECT / "router_adapters" / "reinforce_router.pt")
