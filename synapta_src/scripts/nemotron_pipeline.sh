...
proj = get_shared_projection(
    ...
    rank=64,
    seed=42,
    save_path=Path('$PROJECT_ROOT/adapters/nemotron_30b/shared_projection_B.pt'),
    device='cuda',
...
$VENV -m src.lori_moe.training.train_lori_adapter \
    ...
    --output_dir "$PROJECT_ROOT/adapters/nemotron_30b/adapters" \
...
def find_adapter(domain):
    for sub in ['dare_sparsified', 'best', 'final']:
        p = Path('$PROJECT_ROOT/adapters/nemotron_30b/adapters') / domain / sub
