#!/usr/bin/env python3
"""Held-out eval of the BFSI RECALL LoRA on Nemotron-30B (~214 QAs, no-context).
Modes: base, bfsi_recall_only.
Scoring: substring (case-insensitive), exact match, SQuAD token F1.
Stats: Wilson 95% CI + McNemar (statsmodels, scipy fallback).
Outputs: results/bfsi_recall_eval/eval_results.jsonl + summary.json. Resumes."""
...
EVAL_CLEAN    = PROJECT / "data" / "rbi_corpus" / "qa" / "bfsi_recall" / "eval.jsonl"
EVAL_FALLBACK = PROJECT / "data" / "rbi_corpus" / "qa" / "bfsi_recall" / "eval.jsonl"
OUT_DIR       = PROJECT / "results" / "bfsi_recall_eval"
LOG_FILE      = PROJECT / "logs" / "bfsi_recall_eval" / "eval.log"
...
SYSTEM_MSG = (
    ...
    "Answer the question precisely from your knowledge of Indian financial "
    "regulations (RBI, SEBI, IRDAI). Provide the specific number, term, rule, "
    "or section citation. Be concise."
)
...
def bfsi_aware_router(text):
    t = text.lower()
    return "bfsi_recall" if any(term in t for term in BFSI_TERMS) else code_fallback_router(text)
...
class FormatGuardWithBFSI(LogitsProcessor):
    def __init__(self, model, tok):
        self.model = model
        self.tok = tok
        self.current_adapter = "bfsi_recall"
        self.swap_count = 0
...
def generate(model, base_model, tok, prompt_text, mode):
    ...
    elif mode == "bfsi_recall_only":
        model.set_adapter("bfsi_recall")
        with torch.no_grad():
            out = model.generate(**inputs, **gen_kwargs)
...
def aggregate(out_file, eval_rows, eval_name):
    ...
    pairs = [("bfsi_recall_only", "base"),
             ("format_guard_with_bfsi", "base"),
             ("format_guard_with_bfsi", "bfsi_recall_only")]
    for a, b in pairs:
        av, bv = aligned(a, b)
...
def main():
    ...
    bfsi_ready  = adapter_exists("bfsi_recall")
    other_ready = all(adapter_exists(n) for n in ["math", "code", "science"])
    modes = ["base"]
    if bfsi_ready: modes.append("bfsi_recall_only")
    else: log("WARN: bfsi_recall adapter missing -- skipping bfsi_recall_only.")
    ...
    if bfsi_ready and other_ready:
        ...
        model.load_adapter(adapter_path("bfsi_recall"), adapter_name="bfsi_recall")
    elif bfsi_ready:
        model = PeftModel.from_pretrained(base_model, adapter_path("bfsi_recall"),
                                          adapter_name="bfsi_recall", is_trainable=False)
    else:
        model = base_model
    model.eval()
    ...
    for mode in modes:
        ...
        for q in eval_rows:
            ...
            t0 = time.time()
            try:
                # RECALL MODE: no context — model answers from internalized knowledge
