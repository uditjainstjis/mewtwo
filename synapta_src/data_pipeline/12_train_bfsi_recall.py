...
QA_DIR = PROJECT / "data" / "rbi_corpus" / "qa" / "bfsi_recall"
TRAIN_CLEAN = QA_DIR / "train.jsonl"
TRAIN_FALLBACK = QA_DIR / "train.jsonl"

ADAPTER_ROOT = PROJECT / "adapters" / "nemotron_30b" / "bfsi_recall"
...
LOG_FILE = LOG_DIR / "12_train_recall.log"
for d in (BEST_DIR, TRAIN_DIR, LOG_DIR):
    d.mkdir(parents=True, exist_ok=True)

MAX_LEN = 384  # recall data is short: mean=96 median=93 90th=139 max=224 tokens; 384 covers 100% with headroom for chat template
SEED = 42
EVAL_FRAC = 0.05
...
log = logging.getLogger("12_train_bfsi_recall")
...
def load_examples() -> list[dict]:
    ...
    log.info("Loaded %d valid training examples (recall, no-context)", len(rows))
    return rows


def build_messages(ex: dict) -> list[dict]:
    # RECALL MODE: no context — model must answer from internalized regulatory knowledge
    return [
